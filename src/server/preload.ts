...
const electronAPI = {
  ...
  onUpdateDownloaded: (callback) =>
    ipcRenderer.on('update-downloaded', (event, changelog) =>
      callback(changelog),
    ),
...
const generalAPI = {
  ...
  tutorial: {
    ...
    initializeConfigurations: () =>
      ipcRenderer.invoke('initialize-configurations'),
  },
  ...
  modOperations: {
    renameMod: (oldName, newName) =>
      ipcRenderer.invoke('rename-mod', { oldName, newName }),
    ...
    deleteModFile: (modPath, filePath) =>
      ipcRenderer.invoke('delete-mod-file', { modPath, filePath }),

    writeModFile: (filePath, content) =>
      ipcRenderer.invoke('write-mod-file', { filePath, content }),
  ...
  pluginOperations: {
    ...
    renamePlugin: (oldName, newName) =>
      ipcRenderer.invoke('rename-plugin', { oldName, newName }),
  ...
  settings: {
    ...
    getCustomCssEnabled: () =>
      ipcRenderer.invoke('get-custom-css-enabled').catch((error) => {
        console.error('Failed to get custom CSS enabled state:', error);
      }),

    setCustomCssEnabled: (enabled) =>
      ipcRenderer.invoke('set-custom-css-enabled', enabled).catch((error) => {
        console.error('Failed to set custom CSS enabled state:', error);
      }),

    loadCustomCss: (path) =>
      ipcRenderer.invoke('load-custom-css', path).catch((error) => {
        console.error('Failed to load custom CSS:', error);
      }),

    removeCustomCss: () =>
      ipcRenderer.invoke('remove-custom-css').catch((error) => {
        console.error('Failed to remove custom CSS:', error);
      }),

    getConflictCheckEnabled: () =>
      ipcRenderer.invoke('get-conflict-check-enabled'),

    setConflictCheckEnabled: (enabled) =>
      ipcRenderer.invoke('set-conflict-check-enabled', enabled),

    getAutoPrefixRename: () => ipcRenderer.invoke('get-auto-prefix-rename'),

    setAutoPrefixRename: (enabled) =>
      ipcRenderer.invoke('set-auto-prefix-rename', enabled),
    ...
    setSendVersionEnabled: (enabled) =>
      ipcRenderer.invoke('set-send-version-enabled', enabled),

    getProtocolConfirmEnabled: () =>
      ipcRenderer.invoke('get-protocol-confirm-enabled'),

    setProtocolConfirmEnabled: (enabled) =>
      ipcRenderer.invoke('set-protocol-confirm-enabled', enabled),
    ...
    setAprilFoolsEnabled: (enabled) =>
      ipcRenderer.invoke('set-april-fools-enabled', enabled),
  ...
  discordRpc: {
    ...
    setActivity: (activity) =>
      ipcRenderer.invoke('set-discord-rpc-activity', activity),

    updateModCount: (count) =>
      ipcRenderer.invoke('update-discord-rpc-mod-count', count),

    updateModInstallation: () =>
      ipcRenderer.invoke('update-discord-rpc-mod-installation'),
  },
  ...
  emulator: {
    ...
    setSelectedEmulator: (emulator) =>
      ipcRenderer.invoke('set-selected-emulator', emulator),

    getSelectedEmulator: () => ipcRenderer.invoke('get-selected-emulator'),

    setYuzuFullscreen: (enabled) =>
      ipcRenderer.invoke('set-yuzu-fullscreen', enabled),
  ...
  saveModInfo: (modPath, info) =>
    ipcRenderer.invoke('save-mod-info', modPath, info),
