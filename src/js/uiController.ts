...
class UIController {
    ...
    async handleGameBananaDownload() {
        const linkInputs = document.querySelectorAll<HTMLInputElement>(".gameBananaLink");
    ...
    async performSearch() {
        const searchTerm =
            (document.getElementById("modSearchInput") as HTMLInputElement)?.value.toLowerCase() || "";
        const showEnabled =
            (document.getElementById("enabledFilter") as HTMLInputElement)?.checked || false;
        const showDisabled =
            (document.getElementById("disabledFilter") as HTMLInputElement)?.checked || false;
    ...
    initializeDarkMode() {
        ...
        document.querySelectorAll(".theme-option").forEach((option: HTMLElement) => {
        ...
        window.electron.onProtocolUrl(async (data) => {
            try {
                ...
                if (shouldInstall) {
                    ...
                    const result = await window.electron.downloadMod(parsedUrl.downloadLink);
    ...
    initializeVerticalResizer() {
        ...
        const modPreview = document.querySelector(".mod-preview") as HTMLElement | undefined;
        const modMetadata = document.querySelector(".mod-metadata") as HTMLElement | undefined;
        ...
        try {
            ...
            if (!result.canceled) {
                ...
                if (filePath.toLowerCase().endsWith(".fpp")) {
                    ...
                    const installedMod = await this.modManager.installMod(
                        filePath
                    );
                    if (installedMod) {
    ...
    async handleSelectModsFolder() {
        try {
            ...
            if (!result.canceled) {
                ...
                (document.getElementById("modsPath") as HTMLInputElement).value = result.filePaths[0];
                await this.loadMods();
    ...
    async generateModPrefix(modPath) {
        try {
            ...
            const defaultNames = await ChangeSlots.getDefaultCustomNames(fighterNameInternal);
    ...
    async promptDialog(title: string, message: string, defaultValue: string): Promise<string | null> {
        return new Promise((resolve) => {
    ...
    updateLoadingMessage(message) {
        const messageElement = document.querySelector("#loadingMessage") as HTMLInputElement | undefined;
    ...
    async showConfirmationModal(message) {
        return new Promise((resolve) => {
            ...
            (document.querySelector("#confirmationModal .btn-secondary") as HTMLElement).onclick =
                () => {
                    resolve(false);
                    confirmationModal.hide();
                };
    ...
    async updateModPreview(modId?: string) {
        ...
        const modPreview = document.querySelector(".mod-preview") as HTMLElement | undefined;
        const modImage = document.getElementById("modImage") as HTMLImageElement | undefined;
    ...
    async handleSelectPluginsFolder() {
        try {
            ...
            if (!result.canceled) {
                ...
                (document.getElementById("pluginsPath") as HTMLInputElement).value = result.filePaths[0];
                this.loadPlugins();
    ...
    initializeSettingsTab() {
        ...
        const volumeControl = document.getElementById("volumeControl") as HTMLInputElement | undefined;
    ...
    initializeDiscordRpcToggle() {
        const discordRpcToggle = document.getElementById("discordRpcToggle") as HTMLInputElement | undefined;
    ...
    initializeConflictCheckToggle() {
        const conflictCheckToggle = document.getElementById("conflictCheckToggle") as HTMLInputElement | undefined;
    ...
    initializeAutoPrefixRenameToggle() {
        const autoPrefixRenameToggle = document.getElementById("autoPrefixRenameToggle") as HTMLInputElement | undefined;
    ...
    async handleSelectCustomCssFile() {
        try {
            ...
            if (filePath) {
                (document.getElementById("customCssPath") as HTMLInputElement).value = filePath;
...
`;
        ...
        `;
            ...
            document.querySelectorAll(".target-slot").forEach((select) => {
                select.addEventListener("change", function (this: HTMLSelectElement) {
                    const customInput =
                        this.parentElement.querySelector(".custom-slot-input") as HTMLInputElement;
            ...
            document.querySelectorAll(".copy-to-all-btn").forEach((btn) => {
                btn.addEventListener("click", function (this: HTMLButtonElement) {
                    ...
                    const cspNameElement = sourceSection.querySelector(".custom-csp-name") as HTMLInputElement;
                    const vsNameElement = sourceSection.querySelector(".custom-vs-name") as HTMLInputElement;
                    const boxingRingElement = sourceSection.querySelector(".custom-boxing-ring") as HTMLInputElement;
                    const announcerElement = sourceSection.querySelector(".custom-announcer") as HTMLInputElement;
            ...
            confirmBtn.onclick = async () => {
                try {
                    ...
                    document.querySelectorAll(".target-slot").forEach((select: HTMLSelectElement) => {
                        ...
                        if (targetSlot === "custom") {
                            const customInput =
                                select.parentElement.querySelector(".custom-slot-input") as HTMLInputElement;
                        ...
                        const cspNameElement = slotGroup.querySelector(".custom-csp-name") as HTMLInputElement;
                        const vsNameElement = slotGroup.querySelector(".custom-vs-name") as HTMLInputElement;
                        const boxingRingElement = slotGroup.querySelector(".custom-boxing-ring") as HTMLInputElement;
                        const announcerElement = slotGroup.querySelector(".custom-announcer") as HTMLInputElement;
            ...
            // Handle slot removal
            document.querySelectorAll(".remove-slot").forEach((button: HTMLButtonElement) => {
                button.onclick = () => {
    ...
    async handleChangeSlots(modPath, slotChanges, slotsToRemove, finalSlots: Array<string>, pathData, fighterName, slotCustomNames, defaultCustomNames) {
        try {
    ...
    initializeEmulatorSettings() {
        ...
        const emulatorPath = document.getElementById("emulatorPath") as HTMLInputElement;
        const gamePath = document.getElementById("gamePath") as HTMLInputElement;
        const yuzuFullscreen = document.getElementById("yuzuFullscreen") as HTMLInputElement;
        const yuzuOptions = document.getElementById("yuzuOptions") as HTMLDivElement;
        const selectEmulatorPath = document.getElementById("selectEmulatorPath") as HTMLButtonElement;
        const selectGamePath = document.getElementById("selectGamePath") as HTMLButtonElement;
        const launchGame = document.getElementById("launchGame") as HTMLButtonElement;
    ...
    completeDownload(id, type, message, modName) {
        try {
            ...
            const progressBar = item.querySelector(".progress-bar") as HTMLDivElement;
            const modNameEl = item.querySelector(".mod-name");
            const downloadCount = document.getElementById("downloadCount") as HTMLSpanElement | undefined;
    ...
    async applyAprilFoolsMode() {
        ...
        // Change logo
        const appLogo = document.getElementById("appLogo") as HTMLImageElement | undefined;
    ...
    initializeModSelection() {
        ...
        window.addEventListener("select-mod-from-character", async (event: CustomEvent) => {
    ...
    async createFpp() {
        try {
            const name = (document.getElementById("fppName") as HTMLInputElement).value.trim();
    ...
    initializeFppHandlers() {
        ...
        document.getElementById("fppModsList")?.addEventListener("change", () => {
            const modCheckboxes = document.querySelectorAll(".fpp-mod");
            const selectAllMods = document.getElementById("selectAllMods") as HTMLInputElement;
        ...
        document
            ...
            ?.addEventListener("change", () => {
                const pluginCheckboxes = document.querySelectorAll(".fpp-plugin");
                const selectAllPlugins = document.getElementById("selectAllPlugins") as HTMLInputElement;
                ...
                if (selectAllPlugins) {
                    ...
                    selectAllPlugins.indeterminate =
                        ...
                        Array.from(pluginCheckboxes).some((cb: HTMLInputElement) => cb.checked);
                }
            });
...
async function initializeUI() {
    ...
    languageSelect.addEventListener("change", async (e) => {
        await languageService.loadTranslations((e.target as HTMLSelectElement).value);
    });
}
