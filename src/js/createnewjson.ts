...
export function reslotFighterFiles(modDir, currentAlt, targetAlt, shareSlot, outDir, fighterName, echoColorStart, numColorSlots, baseEchoSlot) {
    const newDirInfos = [];
    ...
    for (let i = 0; i < numColorSlots; i++) {
        ...
        newDirInfosBase[`fighter/${fighterName}/${slot}/camera`] = `fighter/${fighterName}/c${baseEchoSlot}/camera`;
        newDirInfosBase[`fighter/${fighterName}/kirbycopy/${slot}/bodymotion`] = `fighter/${fighterName}/kirbycopy/c${baseEchoSlot}/bodymotion`;
        newDirInfosBase[`fighter/${fighterName}/kirbycopy/${slot}/cmn`] = `fighter/${fighterName}/kirbycopy/c${baseEchoSlot}/cmn`;
        newDirInfosBase[`fighter/${fighterName}/kirbycopy/${slot}/sound`] = `fighter/${fighterName}/kirbycopy/c${baseEchoSlot}/sound`;
        newDirInfosBase[`fighter/${fighterName}/${slot}/cmn`] = `fighter/${fighterName}/c${baseEchoSlot}/cmn`;
    }
...
// Process files from combined shares and add them to the appropriate directories
    Object.entries(combinedShares).forEach(([baseFile, sharedFiles]) => {
        (sharedFiles as any).forEach((file) => {
            ...
            } else if (file.startsWith(`fighter/kirby/model/copy_${fighterName}_`) || file.startsWith(`fighter/kirby/motion/copy_${fighterName}_`)) {
...
// Process echo mod files and add them to `new-dir-files`
    const echoDirs = ['camera', 'kirbycopy', 'movie', 'result', 'model', 'motion', 'sound', 'effect'];
            ...
            echoFiles.forEach((file) => {
                ...
                if (match) {
                    ...
                    if (relativePath.startsWith(`camera/fighter/${fighterName}/`)) {
                        dirPath = `fighter/${fighterName}/camera/${cValue}`;
                    } else if (relativePath.startsWith(`fighter/kirby/model/copy_${fighterName}_`)) {
                        dirPath = `fighter/${fighterName}/kirbycopy/${cValue}`;
                    } else if (relativePath.startsWith(`fighter/${fighterName}/movie/`)) {
                        dirPath = `fighter/${fighterName}/movie/${cValue}`;
                    } else if (relativePath.startsWith(`fighter/${fighterName}/result/`)) {
                        dirPath = `fighter/${fighterName}/result/${cValue}`;
                    } else if (
...
export function main(modDirectory, hashesFile, fighterName, currentAlt, targetAlt, shareSlot, outDir, echoColorStart, numColorSlots, baseEchoSlot) {
