...
class UIController {
  ...
  constructor() {
    try {
      ...
      this.showError(error, 'Failed to initialize UI');
    }
  }
  ...
  async loadSettings() {
    try {
      ...
      this.showError(error, 'Failed to load settings, error');
    }
  }
  ...
  async loadMods() {
    ...
    try {
      ...
      this.showError(error, `Failed to load mods`);
    } finally {
  ...
  initializeEventListeners() {
    ...
    window.electron.onProtocolUrl(async (data) => {
      try {
        ...
        this.showError(error, 'Failed to download mod');
      } finally {
        this.hideLoading();
      }
    });
    ...
    const handleFiles = async (files: FileList) => {
      ...
      if (validFiles.length === 0) {
        this.showError(
          new Error('No valid mod or plugin files found'),
      ...
      try {
        ...
        for (const file of validFiles) {
          ...
          if (ext === 'nro') {
            ...
            const result =
              await window.api.pluginOperations.installPlugin(filePath);

            installResults.push({ fileName: file.name, success: true, result });
          } else {
            // Handle other archive files as mods
            const result = await this.modManager.installMod(filePath);
            installResults.push({ fileName: file.name, success: true, result });
          }
        }
        ...
        this.showError(error, 'Failed to install files');
      } finally {
        this.hideLoading();
      }
    };
    ...
    if (enableAllModsBtn) {
      enableAllModsBtn.addEventListener('click', async () => {
        try {
          ...
          this.showError(error, 'Failed to enable all mods');
        } finally {
          this.hideLoading();
        }
      });
    ...
    if (disableAllModsBtn) {
      disableAllModsBtn.addEventListener('click', async () => {
        try {
          ...
          this.showError(error, 'Failed to disable all mods');
        } finally {
          this.hideLoading();
        }
      });
  ...
  provideInstallationFeedback(results) {
    ...
    if (failedInstalls.length > 0) {
      ...
      this.showError(
        new Error('Installation errors occurred'),
  ...
  async handleInstallMod() {
    try {
      const result = await window.api.dialog.showOpenDialog({
        filters: [
          { name: 'Mod Files', extensions: ['zip', '7z', 'rar', 'fpp'] },
        ],
        properties: ['openFile'],
      });
      ...
      if (!result.canceled) {
        ...
        if (filePath.toLowerCase().endsWith('.fpp')) {
          ...
          if (importResult.success) {
            ...
            this.showError(
              new Error(importResult.error),
              'Failed to import FPP',
            );
          }
        } else {
      ...
      this.showError(error, 'Failed to install');
    } finally {
      this.hideLoading();
    }
  }
  ...
  async handleUninstallMod() {
    ...
    if (!this.selectedMod) {
      this.showError(
        new Error('No mod selected'),
        'Please select a mod to uninstall',
      );
      return;
    }
    ...
    if (
      ...
      try {
        ...
        this.showError(error, 'Failed to uninstall mod');
      } finally {
        this.hideLoading();
      }
    }
  ...
  async handleOpenFolder() {
    try {
      ...
      this.showError(error, 'Failed to open mods folder');
    }
  }
  ...
  async handleReloadList() {
    try {
      ...
      this.showError(error, 'Failed to reload mods, Cause ' + '');
    } finally {
      this.hideLoading();
    }
  }
  ...
  async handleBatchChangeSlots() {
    try {
      ...
      if (!this.mods || this.mods.length === 0) {
        this.showError(
          new Error('No mods available'),
          'No mods available. Please add some mods first.',
        );
        return;
      }
      ...
      for (let i = 0; i < this.mods.length; i++) {
        ...
        try {
          ...
          await this.waitForModalClose('renameModal');
        } catch (error) {
          console.error(`Error processing mod ${mod.name}:`, error);
      ...
      this.showError(error, 'Failed to process batch slot changes');
  ...
  waitForModalClose(modalId: string) {
  ...
  async handleSelectModsFolder() {
    try {
      ...
      this.showError(error, 'Failed to update mods folder');
    } finally {
      this.hideLoading();
    }
  }
  ...
  async handleToggleMod() {
    ...
    try {
      ...
      this.showError(error, 'Failed to toggle mod, Cause ' + '');
    } finally {
      this.hideLoading();
      this.hideContextMenu();
    }
  ...
  async handleOpenModFolder() {
    ...
    try {
      ...
      this.showError(error, 'Failed to open mod folder');
    } finally {
      this.hideContextMenu();
    }
  }
  ...
  async handleRenameMod(skipRenameOnPrefixMatch = false) {
    ...
    if (!this.selectedMod) {
      this.showError(new Error('No mod selected'), 'No mod selected');
      return;
    }
    ...
    try {
      ...
      if (trimmedNewName && trimmedNewName !== currentName) {
        ...
        if (!this.isValidModName(trimmedNewName)) {
          this.showError(new Error('Invalid mod name'), 'Invalid mod name');
          return;
        }
        ...
        try {
          ...
          this.showError(renameError, `Failed to rename mod`);
        } finally {
          this.hideLoading();
        }
      }
  ...
  async showLoading(message = 'Loading...') {
    ...
    //         this.showError(error, 'Failed to cancel download');
    //       }
    //     });
    // }
  }
  ...
  showError(error: Error, customMessage: string) {
    ...
    try {
      ...
      newSaveButton.onclick = async () => {
        ...
        try {
          ...
          this.showError(error, 'Failed to save mod info');
        }
      };
      ...
      this.showError(error, 'Failed to load mod info');
    }
  }

  async selectMod(modId: string) {
  ...
  async loadPlugins() {
    try {
      ...
      this.showError(error, 'Failed to load plugins');
    }
    this.hideLoading();
  }
  ...
  async handleSelectPluginsFolder() {
    try {
      ...
      this.showError(error, 'Failed to update plugins folder');
    } finally {
      this.hideLoading();
    }
  }
  ...
  async handleInstallPlugin() {
    try {
      ...
      this.showError(error, 'Failed to install plugin');
    } finally {
      this.hideLoading();
    }
  }
  ...
  async handleDeletePlugin(pluginId) {
    if (
      ...
      try {
        ...
        this.showError(error, 'Failed to delete plugin');
      } finally {
        this.hideLoading();
      }
    }
  ...
  async handleTogglePlugin(pluginId) {
    try {
      ...
      this.showError(error, 'Failed to toggle plugin');
    }
  }
  ...
  async handleOpenPluginsFolder() {
    try {
      ...
      this.showError(error, 'Failed to open plugins folder, Cause');
    }
  }
  ...
  async handlePluginDrop(event: DragEvent) {
    ...
    if (nroFiles.length === 0) {
      this.showError(
        new Error('No .nro files found'),
        'No valid plugin files found. Please drop .nro files.',
      );
      return;
    }
    ...
    try {
      ...
      for (const file of nroFiles) {
        ...
        try {
          ...
          this.showError(installError, `Failed to install ${file.name}`);
        }
      }
      ...
      this.showError(error, 'Failed to install plugins');
    } finally {
      this.hideLoading();
    }
  }
    ...
    document
      ...
      .addEventListener('click', async () => {
        try {
          ...
          this.showError(error, 'Failed to clear temporary files');
        } finally {
          this.hideLoading();
        }
      });
  ...
  async initializeLogViewer() {
    try {
      ...
      this.showError(error, 'Failed to load logs');
    }
  }
  ...
  async handleOpenCurrentLog() {
    try {
      ...
      this.showError(error, 'Failed to open current log');
    }
  }
  ...
  async handleOpenLogsFolder() {
    try {
      ...
      this.showError(error, 'Failed to open logs folder');
    }
  }
  ...
  async handleRefreshLogs() {
    try {
      ...
      this.showError(error, 'Failed to refresh logs');
    }
  }
  ...
  async handleClearLogs() {
    try {
      ...
      this.showError(error, 'Failed to clear logs');
    }
  }
  ...
  handleRemoveCustomCssFile() {
    ...
    window.api.settings.removeCustomCss().catch((error) => {
      this.showError(error, 'Failed to remove custom CSS');
    });
  ...
  async handleSelectCustomCssFile() {
    try {
      const filePath = await window.electron.selectCustomCssFile();

      if (filePath) {
        (document.getElementById('customCssPath') as HTMLInputElement).value =
          filePath;

        await window.electron.setCustomCssPath(filePath);

        this.showSuccess('Custom CSS file updated successfully');
        this.showRestartNeededPopup();
      }
    } catch (error) {
      this.showError(error, 'Failed to select custom CSS file');
    }
  }
...
`;
  ...
  async showChangeSlotsDialog(selectedMod: string) {
    ...
    if (!selectedMod) {
      this.showError(new Error(), 'Please select a mod');
      return;
    }
    ...
    try {
      ...
      confirmBtn.onclick = async () => {
        try {
          ...
          } else if (!this.isBatchProcessing) {
            await this.loadMods();
          }
        } catch (error) {
          this.showError(error, `Failed to change slots`);
        }
      };
      ...
      this.showError(error, `Failed to scan mod`);
    } finally {
      this.hideLoading();
    }
  }
  ...
  initializeEmulatorSettings() {
    ...
    launchGame.addEventListener('click', async () => {
      try {
        ...
        this.showError(error, `Failed to launch game`);
      }
    });
  }
  ...
  async pauseDownload(id) {
    try {
      ...
      this.showError(error, 'Failed to pause/resume download');
    }
  }
  ...
  async showCreateFppDialog() {
    try {
      ...
      this.showError(error, 'Failed to load mods/plugins');
    }
  }
  ...
  async createFpp() {
    try {
      ...
      if (!name) {
        this.showError(new Error(), 'Please enter a package name');
        return;
      }
      ...
      if (result.success) {
        ...
        this.showError(result, 'Failed to create package');
      }
    } catch (error) {
      this.showError(error, 'Failed to create package');
    } finally {
      this.hideLoading();
    }
  }
  ...
  async handleImportFpp(filePath) {
    try {
      ...
      if (result.success) {
        ...
        this.showError(result, 'Failed to import package');
      }
    } catch (error) {
      this.showError(error, 'Failed to import package');
    } finally {
      this.hideLoading();
    }
  }
  ...
  async handleImportFppClick() {
    try {
      ...
      this.showError(error, 'Failed to import FPP');
    }
  }
    ...
    try {
      ...
      for (const file of files) {
        ...
        if (result.success) {
          ...
          this.showError(result.error, 'Failed to import package');
        }
      }
    } catch (error) {
      this.showError(error, 'Import error');
    } finally {
      this.hideLoading();
    }
  }
    ...
    if (legacyModDiscoveryToggle) {
      ...
      legacyModDiscoveryToggle.addEventListener('change', async (e) => {
        try {
          ...
          this.showError(
            error,
            'Failed to update legacy mod discovery setting',
          );
        }
      });
    } else {
...
window.uiController = { selectCustomCssFile };
